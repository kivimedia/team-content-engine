...
class FounderVoiceExtractor(AgentBase):
    ...
    async def _execute(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        self._report(f"Voice profile extracted from {len(all_profiles)} chunks:")
        vocab = merged.get("vocabulary_signature", {})
        words = vocab.get("distinctive_words", [])
        if words:
            self._report(f"  Distinctive words: {', '.join(str(w) for w in words[:10])}")
        phrases = vocab.get("phrases", [])
        if phrases:
            self._report(f"  Key phrases: {', '.join(str(p) for p in phrases[:5])}")
        values = merged.get("values_and_beliefs", [])
        if values:
            self._report(f"  Values ({len(values)}): {', '.join(str(v) for v in values[:5])}")
        metaphors = merged.get("metaphor_families", [])
        if metaphors:
            self._report(f"  Metaphor families: {', '.join(str(m) for m in metaphors[:5])}")
        humor = merged.get("humor_type", "")
        if humor:
            self._report(f"  Humor type: {humor}")
        tone = merged.get("tone_range", {})
        if tone:
            self._report(f"  Tone range: {json.dumps(tone)}")
        themes = merged.get("recurring_themes", [])
        if themes:
            self._report(f"  Recurring themes: {', '.join(str(t) for t in themes[:5])}")
        taboos = merged.get("taboos", [])
        if taboos:
