...
class LearningLoop(AgentBase):
    ...
    async def _execute(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        summary = recommendations.get("week_summary", "")
        if summary:
            self._report(f"Week summary: {summary}")
        template_recs = recommendations.get("template_recommendations", [])
        if template_recs:
            self._report(f"Template recommendations ({len(template_recs)}):")
            for r in template_recs[:5]:
                name = r.get("template_name", "unknown") if isinstance(r, dict) else str(r)
                action = r.get("action", "") if isinstance(r, dict) else ""
                self._report(f"  - {name}: {action}")
        cta_recs = recommendations.get("cta_recommendations", [])
        if cta_recs:
            self._report(f"CTA recommendations ({len(cta_recs)}):")
            for r in cta_recs[:3]:
                keyword = r.get("keyword", "") if isinstance(r, dict) else str(r)
                rec = r.get("recommendation", "") if isinstance(r, dict) else ""
                self._report(f"  - {keyword}: {rec}")
        actions = recommendations.get("action_items", [])
        if actions:
            self._report(f"Action items ({len(actions)}):")
            for a in actions[:5]:
                self._report(f"  - {str(a)[:100]}")
