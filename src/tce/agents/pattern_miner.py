...
class PatternMiner(AgentBase):
    ...
    async def _execute(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        self._report(f"Mined {len(templates)} templates:")
        for i, t in enumerate(templates, 1):
            name = t.get("template_name", "unnamed")
            family = t.get("template_family", "unknown")
            best_for = t.get("best_for", "")
            self._report(f"  {i}. {name} [{family}]")
            if best_for:
                self._report(f"     Best for: {str(best_for)[:100]}")
            hook = t.get("hook_formula", "")
            if hook:
                self._report(f"     Hook: {str(hook)[:100]}")
