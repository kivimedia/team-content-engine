...
class PipelineOrchestrator:
    ...
    async def run(
        ...
        If resume_from_step is provided, all steps before that step in the
        dependency chain are marked COMPLETED (assumed done in a prior run).
        Execution starts from the named step.
        """
        if initial_context:
    ...
    async def _persist_step_result(self, step_name: str) -> None:
        ...
        try:
            if step_name == "corpus_analyst":
                ...
                if resolved_tpl:
                    self.context["_resolved_template"] = resolved_tpl
            ...
            elif step_name == "script_agent":
                await self._persist_narration_script()
    ...
    async def _persist_narration_script(self) -> None:
        """Save the narration script generated by script_agent to the database."""
        from tce.models.narration_script import NarrationScript

        script_data = self.context.get("narration_script", {})
        if not script_data or not script_data.get("segments"):
            return

        pkg_id = self.context.get("_post_package_id")
        if not pkg_id:
            logger.warning("narration_script.no_package_id", run_id=str(self.run_id))
            return
        ...
        self.db.add(ns)
        await self.db.flush()
        self.context["_narration_script_id"] = str(ns.id)
        logger.info("narration_script.saved", script_id=str(ns.id), package_id=str(pkg_id))
    ...
    def _mark_predecessors_completed(self, target_step: str) -> None:
        """Mark all steps that are predecessors of target_step as COMPLETED.

        This enables resuming a pipeline from a specific step by treating
        prior steps as already done.
        """
        predecessors: set[str] = set()

        def _collect(step_name: str) -> None:
            step = self.steps.get(step_name)
            if not step:
                return
            for dep in step.depends_on:
                if dep not in predecessors and dep in self.steps:
                    predecessors.add(dep)
                    _collect(dep)

        _collect(target_step)

        for name in predecessors:
            self.step_status[name] = StepStatus.COMPLETED
