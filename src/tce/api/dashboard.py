...
:root{--bg:#0f1117;--card:#1a1d27;--border:#2a2d3a;--text:#e4e4e7;--dim:#9ca3af;--accent:#6366f1;
--accent2:#818cf8;--green:#22c55e;--red:#ef4444;--yellow:#eab308;--blue:#3b82f6}
body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:var(--bg);
color:var(--text);min-height:100vh}
.header{background:var(--card);border-bottom:1px solid var(--border);padding:16px 24px;display:flex;
...
.nav{display:flex;gap:4px;background:var(--card);padding:8px 24px;
border-bottom:1px solid var(--border)}
.nav button{padding:8px 16px;border:none;background:transparent;color:var(--dim);cursor:pointer;
...
.card h3{font-size:14px;color:var(--dim);margin-bottom:12px;text-transform:uppercase;
letter-spacing:.5px}
...
select,input{padding:8px 12px;border:1px solid var(--border);background:var(--card);
color:var(--text);border-radius:6px;font-size:13px}
.pipeline-steps{display:flex;gap:8px;flex-wrap:wrap;margin:16px 0}
.step-badge{padding:6px 12px;border-radius:16px;font-size:12px;font-weight:500;
...
.plan-step{padding:4px 12px;border-radius:12px;font-size:12px;font-weight:500;
...
.post-preview{background:#111318;border:1px solid var(--border);border-radius:8px;padding:16px;
margin:8px 0;white-space:pre-wrap;font-size:14px;line-height:1.6;max-height:300px;overflow-y:auto}
...
.tabs button{padding:4px 10px;border:1px solid var(--border);background:transparent;
...
.doc-row{display:flex;align-items:center;justify-content:space-between;padding:12px 16px;
...
.log{background:#111318;border:1px solid var(--border);border-radius:8px;padding:12px;
font-family:monospace;font-size:12px;max-height:200px;overflow-y:auto;line-height:1.5;
color:var(--dim)}
.empty{text-align:center;padding:40px;color:var(--dim)}
.toast{position:fixed;bottom:24px;right:24px;padding:12px 20px;border-radius:8px;font-size:13px;
font-weight:500;z-index:999;animation:slideIn .3s ease}
.toast-ok{background:var(--green);color:#000}
.toast-err{background:var(--red);color:#fff}
@keyframes slideIn{from{transform:translateY(20px);opacity:0}to{transform:translateY(0);opacity:1}}
.voice-profile{background:#111318;border:1px solid var(--border);border-radius:8px;padding:16px;
margin:8px 0}
...
.day-card{background:var(--card);border:1px solid var(--border);border-radius:10px;padding:16px;
...
.day-card .day-angle{font-size:12px;color:var(--accent2);background:#1e1b4b;padding:3px 8px;
border-radius:4px;display:inline-block;margin-bottom:8px;position:relative}
.tip-icon{display:inline-flex;align-items:center;justify-content:center;width:14px;height:14px;
border-radius:50%;background:var(--accent2);color:#1e1b4b;font-size:9px;font-weight:700;
font-style:italic;margin-left:4px;cursor:help;vertical-align:middle}
.tip-icon:hover::after{content:attr(data-tip);position:absolute;left:0;top:calc(100% + 6px);
width:260px;padding:10px 12px;background:#1e1b4b;border:1px solid var(--accent);color:var(--text);
font-size:12px;font-style:normal;font-weight:400;border-radius:8px;z-index:99;line-height:1.5;
white-space:normal;box-shadow:0 4px 16px rgba(0,0,0,.5)}
.day-card .day-topic{font-size:13px;color:var(--text);margin-bottom:8px;flex:1}
.day-card .day-status{font-size:11px;font-weight:600;text-transform:uppercase;padding:2px 6px;
...
.guide-card{background:var(--card);border:1px solid var(--border);border-radius:10px;padding:20px;
margin-top:16px}
...
<div class="header">
  ...
  <div class="status"><div class="dot" id="health-dot"></div><span id="health-text">
  Checking...</span></div>
</div>
...
let genAllState = null;
// {running, current, total, startTime, results: [{day, status, stepStatus, startTime}]}
const AGENT_LABELS = {
...
async function renderWeek() {
  ...
  app.innerHTML = `
    <div class="section">
      ...
      gap:12px;margin-bottom:16px">
        <h2>Week of ${monday.toLocaleDateString('en-US',{month:'short',
        day:'numeric'})} - ${friday.toLocaleDateString('en-US',{month:'short',day:'numeric',
        year:'numeric'})}</h2>
      ...
      <div style="display:flex;gap:12px;align-items:end;flex-wrap:wrap;margin-bottom:16px">
        <div>
          <label style="font-size:12px;color:var(--dim);display:block;
          margin-bottom:4px">Weekly Theme (optional hint for the planner)</label>
          <input id="week-theme" type="text" placeholder="e.g. Agency scaling without burnout"
          style="width:320px">
        </div>
      ...
      <div class="week-grid" id="week-grid"><div class="empty" style="grid-column:1/-1">
      Loading calendar...</div></div>
  ...
  try {
    ...
    if (weeklyMeta && weeklyMeta.weekly_theme) {
      ...
      let summaryHtml = '<div style="background:linear-gradient(135deg,#1a1d27,#1e2235);
      border:1px solid var(--accent);border-radius:10px;padding:16px 20px;margin-bottom:16px;
      display:flex;gap:20px;flex-wrap:wrap;align-items:flex-start">';
      summaryHtml += '<div style="flex:1;min-width:200px">';
      summaryHtml += '<div style="font-size:11px;text-transform:uppercase;letter-spacing:1px;
      color:var(--accent2);margin-bottom:4px">Weekly Direction</div>';
      summaryHtml += '<div style="font-size:15px;font-weight:600;
      line-height:1.4">' + escHtml(weeklyMeta.weekly_theme) + '</div>';
      summaryHtml += '</div>';
      if (giftTitle) {
        summaryHtml += '<div style="flex:0 0 auto;background:rgba(34,197,94,0.08);
        border:1px solid rgba(34,197,94,0.3);border-radius:8px;padding:10px 14px;min-width:180px">';
        summaryHtml += '<div style="font-size:11px;text-transform:uppercase;letter-spacing:1px;
        color:var(--green);margin-bottom:4px">Gift of the Week</div>';
        summaryHtml += '<div style="font-size:14px;
        font-weight:600">' + escHtml(giftTitle) + '</div>';
        if (giftSubtitle) summaryHtml += '<div style="font-size:12px;color:var(--dim);
        margin-top:2px">' + escHtml(giftSubtitle) + '</div>';
        if (sections.length) summaryHtml += '<div style="font-size:11px;color:var(--dim);
        margin-top:6px">' + sections.length + ' sections planned</div>';
        summaryHtml += '</div>';
      }
      if (cta) {
        summaryHtml += '<div style="flex:0 0 auto;text-align:center;padding:10px 14px">';
        summaryHtml += '<div style="font-size:11px;text-transform:uppercase;letter-spacing:1px;
        color:var(--yellow);margin-bottom:4px">CTA Keyword</div>';
        summaryHtml += '<div style="font-size:20px;font-weight:800;
        color:var(--yellow)">' + escHtml(cta) + '</div>';
        summaryHtml += '</div>';
      }
      summaryHtml += '<div style="flex:0 0 auto;display:flex;
      align-items:center"><button class="btn btn-dim" style="padding:8px 14px;font-size:12px;
      white-space:nowrap" onclick="editWeekPlan()">Edit Plan</button></div>';
      summaryHtml += '</div>';
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const angle = entry ? entry.angle_type : ['big_shift_explainer','tactical_workflow_guide',
      'contrarian_diagnosis','case_study_build_story','second_order_implication'][i];
      ...
      html += '<div class="day-date">' + d.toLocaleDateString('en-US',{month:'short',
      day:'numeric'}) + '</div>';
      const tip = ANGLE_TIPS[angle] || '';
      html += '<div class="day-angle" style="position:relative;
      cursor:help">' + (ANGLE_LABELS[angle] || angle.replace(/_/g,' '));
      if (tip) html += ' <span class="tip-icon" data-tip="' + esc(tip) + '">i</span>';
      html += '</div>';
      if (entry?.topic) html += '<div class="day-topic">' + esc(entry.topic) + '</div>';
      else html += '<div class="day-topic" style="color:var(--dim);
      font-style:italic">No topic set</div>';
      if (entry?.operator_notes) html += '<div style="font-size:11px;color:var(--dim);
      margin-bottom:6px">' + esc(entry.operator_notes) + '</div>';

      html += '<div style="display:flex;justify-content:space-between;align-items:center;
      margin-top:auto;gap:4px;flex-wrap:wrap">';
      if (entry) {
        html += '<span class="day-status day-status-' + status + '">' + status.toUpperCase()
        + '</span>';
        if (status === 'planned') {
          html += '<button class="btn btn-primary" style="padding:4px 10px;
          font-size:11px" onclick="runDayPipeline(' + i + ',
          \\'' + (entry?.id || '') + '\\')">Generate</button>';
        } else if (entry.post_package_id) {
          html += '<button class="btn btn-green" style="padding:4px 10px;
          font-size:11px" onclick="viewPackage(\\'' + entry.post_package_id + '\\')
          ">View Package</button>';
        }
      } else {
        html += '<span class="day-status" style="background:var(--border);
        color:var(--dim)">UNPLANNED</span>';
      }
      html += '</div>';
      html += '</div>';
    }
    ...
    document.getElementById('week-grid')
    .innerHTML = '<div class="empty" style="grid-column:1/-1">
    Error loading calendar: ' + e.message + '</div>';
  }
  ...
  try {
    ...
    ''));
    if (guides.length) {
      let html = '<h2 style="margin-top:24px;margin-bottom:12px;display:flex;align-items:center;
      gap:16px">Gift of the Week (Weekly Guides)';
      html += '<label style="font-size:12px;font-weight:400;color:var(--dim);display:flex;
      align-items:center;gap:6px;
      cursor:pointer"><input type="checkbox" id="show-archived-guides" onchange="renderWeek()
      " ' + (showArchivedGuides ? 'checked' : '') + '> Show Archived</label>';
      html += '</h2>';
      for (const g of guides) {
        const archived = g.is_archived;
        html += '<div class="guide-card" style="' + (archived ? 'opacity:0.5;
        border-color:var(--dim)' : '') + '">';
        if (archived) html += '<div style="display:inline-block;background:var(--dim);color:#000;
        font-size:10px;font-weight:700;padding:2px 8px;border-radius:4px;
        margin-bottom:8px">ARCHIVED</div>';
        ...
        if (g.docx_path)
        html += '<a class="btn btn-green" href="' + API + '/content/guides/' + g.id + '/download"
        target="_blank">Download DOCX</a>';
        if (g.fulfillment_link) html += '<a class="btn btn-blue" href="' + esc(g.fulfillment_link)
        + '" target="_blank">Fulfillment Link</a>';
        if (archived) {
          html += '<button class="btn btn-dim" onclick="unarchiveGuide(\\'' + g.id + '\\')
          ">Unarchive</button>';
        } else {
          html += '<button class="btn btn-dim" onclick="archiveGuide(\\'' + g.id + '\\')
          ">Archive</button>';
        }
        html += '</div>';
        if (g.markdown_content) {
          html += '<details style="margin-top:8px"><summary style="cursor:pointer;
          color:var(--accent2);font-size:13px">View Guide Content</summary>';
          html += '<div class="post-preview" style="margin-top:8px;max-height:400px;overflow-y:auto;
          white-space:pre-wrap">' + esc(g.markdown_content) + '</div>';
          html += '</details>';
        }
        html += '<div class="guide-stats">';
        html += '<div class="guide-stat"><div class="val">' + (g.downloads_count || 0)
        + '</div><div class="lbl">Downloads</div></div>';
        html += '<div class="guide-stat"><div class="val">
        ' + (g.conversion_rate != null ? (g.conversion_rate * 100).toFixed(1) + '%' : 'N/A')
        + '</div><div class="lbl">Conversion</div></div>';
        html += '</div>';
        html += '<div style="font-size:11px;color:var(--dim);
        margin-top:8px">Created: ' + new Date(g.created_at).toLocaleString() + '</div>';
        html += '</div>';
      }
      document.getElementById('guide-section').innerHTML = html;
    } else {
      document.getElementById('guide-section')
      .innerHTML = '<div class="card" style="margin-top:16px;text-align:center;
      padding:24px"><div style="font-size:14px;
      color:var(--dim)
      ">
      No weekly guides yet. Click "Generate Weekly Guide" to create your first gift of the
      week.</div></div>';
    }
  } catch {}
}
...
async function planWeekDeep(mondayStr) {
  ...
  if (panel) panel.innerHTML = '<div class="card" style="padding:20px;margin-bottom:16px;
  border-left:4px solid var(--accent)">' +
    '<div style="display:flex;align-items:center;gap:12px;margin-bottom:12px">' +
    '<div class="spinner"></div>' +
    '<div style="flex:1"><div id="plan-progress-text" style="font-weight:600">
    Starting weekly planning...</div>' +
    '<div style="font-size:12px;color:var(--dim);
    margin-top:4px">This takes about 60-90 seconds</div></div>' +
    '<div id="plan-elapsed" style="font-size:24px;font-weight:700;color:var(--accent);
    font-variant-numeric:tabular-nums">0s</div>' +
    '</div>' +
    ...
    while (!done) {
      ...
      try {
        ...
        if (st.status === 'completed') {
          ...
          "><b>Planning failed:</b> ' + (st.error || 'Unknown error') + '</div>';
          toast('Planning failed: ' + (st.error || 'Unknown error'), false);
        }
      } catch (e) {
        failCount++;
        if (failCount >= 3) { done = true; if (panel) panel.innerHTML = '';
        toast('Lost connection to planner', false); }
      }
    }
  } catch (e) {
...
function showPlanReview(planData, mondayStr) {
  ...
  approvedPlan = { weekly_theme: wp.weekly_theme || '', gift_theme: giftTheme,
  cta_keyword: wp.cta_keyword || '', days: days };

  let html = '<div style="margin-bottom:24px">';

  // === BIG HEADER: Weekly Direction ===
  html += '<div class="card" style="padding:24px;margin-bottom:16px;border:2px solid var(--accent);
  border-radius:12px">';
  html += '<div style="display:flex;justify-content:space-between;align-items:start;
  margin-bottom:20px">';
  html += '<div><h2 style="margin:0 0 4px 0;color:var(--accent);
  font-size:20px">Weekly Plan Review</h2>';
  html += '<p style="margin:0;color:var(--dim);
  font-size:13px">Edit anything below, then approve to start content generation.</p></div>';
  html += '<div style="text-align:right;font-size:12px;
  color:var(--dim)">Plan ID: ' + (deepPlanId || '').substring(0, 8) + '</div>';
  html += '</div>';

  // Weekly Theme - big and prominent
  html += '<div style="margin-bottom:16px">';
  html += '<label style="font-size:12px;color:var(--dim);display:block;margin-bottom:4px;
  font-weight:600">OVERARCHING DIRECTION - What ties the whole week together?</label>';
  html += '<textarea id="pr-weekly-theme" rows="2" style="width:100%;padding:10px;
  border:1px solid var(--accent);border-radius:6px;background:var(--bg);color:var(--text);
  font-size:15px;resize:vertical">' + escHtml(wp.weekly_theme || '') + '</textarea>';
  html += '</div>';

  // Gift Theme - prominent box
  html += '<div style="background:rgba(34,197,94,0.08);border:1px solid var(--green);
  border-radius:8px;padding:16px;margin-bottom:16px">';
  html += '<div style="display:flex;align-items:center;gap:8px;
  margin-bottom:8px"><span style="font-size:18px">&#127873;</span><label style="font-size:12px;
  color:var(--green);font-weight:700;
  text-transform:uppercase">Weekly Gift / Lead Magnet</label></div>';
  html += '<textarea id="pr-gift-theme" rows="2" style="width:100%;padding:10px;
  border:1px solid var(--green);border-radius:6px;background:var(--bg);color:var(--text);
  font-size:14px;
  resize:vertical">' + escHtml(giftTitle + (giftSubtitle ? ' - ' + giftSubtitle : ''))
  + '</textarea>';
  if (giftSections.length > 0) {
    html += '<div style="margin-top:8px;font-size:12px;
    color:var(--dim)"><b>Guide sections:</b> ' + giftSections.map(s => escHtml(s)).join(' / ')
    + '</div>';
  }
  html += '<div style="display:flex;gap:16px;margin-top:12px">';
  html += '<div style="flex:1"><label style="font-size:11px;color:var(--dim);display:block;
  margin-bottom:4px">CTA Keyword (what readers comment to get the gift)</label>';
  html += '<input id="pr-cta-keyword" type="text" value="' + escHtml(wp.cta_keyword || '')
  + '" style="width:100%;padding:8px;border:1px solid var(--border);border-radius:6px;
  background:var(--bg);color:var(--text);text-transform:uppercase;font-weight:700;font-size:16px;
  ...
  const angleLabels = { big_shift_explainer: 'Big Shift',
  tactical_workflow_guide: 'Tactical How-To', contrarian_diagnosis: 'Contrarian Take',
  case_study_build_story: 'Case Study', second_order_implication: 'Big Picture' };
  const angleColors = { big_shift_explainer: '#6366f1', tactical_workflow_guide: '#22c55e',
  contrarian_diagnosis: '#ef4444', case_study_build_story: '#eab308',
  second_order_implication: '#3b82f6' };
  ...
  for (let i = 0; i < days.length; i++) {
    ...
    html += '<span style="font-size:12px;padding:3px 10px;border-radius:8px;
    background:' + angleColor + '22;color:' + angleColor + ';
    font-weight:600">' + escHtml(angleLabel) + '</span>';
    html += '</div>';
    html += '<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">';
    html += '<div><label style="font-size:11px;color:var(--dim);text-transform:uppercase;
    font-weight:600;display:block;margin-bottom:4px">Topic (what is this post about?)</label>';
    html += '<textarea id="pr-day-' + i + '-topic" rows="2" style="width:100%;padding:8px 10px;
    border:1px solid var(--border);border-radius:6px;background:var(--bg);color:var(--text);
    font-size:14px;resize:vertical;
    line-height:1.4" placeholder="Write like you would describe it to a friend...">
    ' + escHtml(d.topic || '') + '</textarea></div>';
    html += '<div><label style="font-size:11px;color:var(--dim);text-transform:uppercase;
    font-weight:600;display:block;
    margin-bottom:4px">Core Argument (what should the reader believe?)</label>';
    html += '<textarea id="pr-day-' + i + '-thesis" rows="2" style="width:100%;padding:8px 10px;
    border:1px solid var(--border);border-radius:6px;background:var(--bg);color:var(--text);
    font-size:13px;resize:vertical;
    line-height:1.4" placeholder="The main takeaway...">' + escHtml(d.thesis || '')
    + '</textarea></div>';
    html += '</div>';
    html += '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-top:8px">';
    html += '<div><label style="font-size:11px;color:var(--dim);text-transform:uppercase;
    font-weight:600;display:block;margin-bottom:4px">Audience</label>';
    html += '<input id="pr-day-' + i + '-audience" type="text" value="' + escHtml(d.audience || '')
    + '" style="width:100%;padding:6px 10px;border:1px solid var(--border);border-radius:6px;
    background:var(--bg);color:var(--text);font-size:13px" placeholder="Who is this for?"></div>';
    html += '<div><label style="font-size:11px;color:var(--dim);text-transform:uppercase;
    font-weight:600;display:block;margin-bottom:4px">Belief Shift</label>';
    html += '<input id="pr-day-' + i + '-belief" type="text" value="' +
    escHtml(d.desired_belief_shift || '') + '" style="width:100%;padding:6px 10px;
    border:1px solid var(--border);border-radius:6px;background:var(--bg);color:var(--text);
    font-size:13px" placeholder="FROM x TO y"></div>';
    html += '<div><label style="font-size:11px;color:var(--dim);text-transform:uppercase;
    font-weight:600;display:block;margin-bottom:4px">Gift Connection</label>';
    html += '<input id="pr-day-' + i + '-gift" type="text" value="' +
    escHtml(days[i].connection_to_gift || d.connection_to_gift || '') + '" style="width:100%;
    padding:6px 10px;border:1px solid var(--border);border-radius:6px;background:var(--bg);
    color:var(--text);font-size:13px" placeholder="How does this tie to the gift?"></div>';
    html += '</div>';
    html += '<input id="pr-day-' + i + '-visual" type="hidden" value="' + escHtml(d.visual_job ||
    'cinematic_symbolic') + '">';
    html += '</div>';
  }
  html += '</div>';
  ...
  if (trends.length > 0) {
    html += '<details style="margin-bottom:16px"><summary style="cursor:pointer;color:var(--dim);
    font-size:13px;
    padding:8px 0">Trend Brief - ' + trends.length + ' trends found (click to expand)</summary>';
    ...
    trends.forEach(t => {
      ...
      font-size:11px">Relevance: ' + t.relevance_score + '/10';
      if (t.source_url) html += ' - <a href="' + escHtml(t.source_url)
      + '" target="_blank" style="color:var(--accent)">source</a>';
      html += '</div></div>';
    });
    html += '</div></details>';
  }
  ...
  html += '<button class="btn btn-green" style="padding:12px 24px;font-size:15px;
  font-weight:600" onclick="approveAndGenerate(\\'' + mondayStr + '\\')
  ">Approve & Generate All 5 Days</button>';
  html += '<button class="btn btn-primary" style="padding:12px 24px" onclick="savePlanOnly(\\'' +
  mondayStr + '\\')">Save Plan Only</button>';
  html += '<button class="btn btn-dim" style="padding:12px 24px" onclick="dismissPlanReview()
  ">Dismiss</button>';
  html += '</div>';
...
async function generateFromApprovedPlan(plan) {
  genAllState = { running: true, current: -1, total: 5, startTime: Date.now(), results: [],
  totalCost: 0, unified: true, weekId: null, phase: 'starting',
  phaseDetail: 'Starting generation from approved plan...', weeklyTheme: plan.weekly_theme,
  giftTheme: plan.gift_theme, weeklyKeyword: plan.cta_keyword };
  saveGenAllState();
  renderGenAllProgress();
  try {
      ...
      try {
        ...
        if (st.day_run_ids) {
          for (let idx = 0; idx < st.day_run_ids.length; idx++) {
            if (!genAllState.results[idx]) genAllState.results[idx] = { day: DAY_NAMES[idx],
            status: 'done', runId: st.day_run_ids[idx] };
          }
        }
        if (st.phase === 'generating_days' && st.current_day >= 0 && st.day_run_ids) {
          ...
          if (dayRunId) {
            try {
              ...
              if (!genAllState.results[st.current_day])
              genAllState.results[st.current_day] = { day: DAY_NAMES[st.current_day],
              status: 'running' };
              genAllState.results[st.current_day].stepStatus = dayStatus.step_status || {};
            } catch {}
          }
          if (!genAllState.results[st.current_day])
          genAllState.results[st.current_day] = { day: DAY_NAMES[st.current_day], status: 'running',
          stepStatus: {} };
        ...
        pollFailCount++;
        if (pollFailCount >= 5) { genAllState.phase = 'failed';
        genAllState.phaseDetail = 'Lost connection'; done = true; }
      }
      saveGenAllState();
      renderGenAllProgress();
    }
...
').replace(/>/g,'&gt;').replace(/"/g,'&quot;'); }
function dismissPlanReview() { const p = document.getElementById('plan-review-panel');
if (p) p.innerHTML = ''; renderWeek(); }
...
function editWeekPlan() {
  ...
  const days = entries.filter(e => e.plan_context).map(e => {
    ...
    return { day_of_week: e.day_of_week, angle_type: e.angle_type, topic: e.topic || pc.topic || '',
    thesis: pc.thesis || '', audience: pc.audience || '',
    desired_belief_shift: pc.desired_belief_shift || '',
    connection_to_gift: pc.connection_to_gift || '',
    visual_job: pc.visual_job || 'cinematic_symbolic',
    evidence_requirements: pc.evidence_requirements || [],
    platform_notes: pc.platform_notes || '' };
  });
  const planData = { weekly_plan: { weekly_theme: meta.weekly_theme || '',
  gift_theme: meta.gift_theme || '', gift_sections: meta.gift_sections || [],
  cta_keyword: meta.cta_keyword || '', days: days }, trend_summary: [] };
...
function renderGenAllProgress() {
  ...
  if (s.running) {
    if (s.unified) {
      ...
      else if (s.phase === 'generating_days')
      headerText = 'Generating day ' + ((s.current >= 0 ? s.current : 0) + 1) + '/5...';
  ...
  if (elapsed) html += '<div style="font-size:12px;color:var(--dim);
  font-family:monospace">' + elapsed + '</div>';
  html += '</div>';

  // Show weekly theme + gift theme once available (unified mode)
  if (s.unified && s.weeklyTheme) {
    html += '<div style="background:#0d1117;border:1px solid var(--border);border-radius:8px;
    ...
    color:var(--accent2)">' + esc(s.weeklyTheme) + '</div>';
    if (s.giftTheme) {
    ...
    if (s.weeklyKeyword) {
      html += '<div style="font-size:11px;color:var(--dim);
      margin-top:4px">CTA KEYWORD: <span style="color:var(--yellow);
      font-weight:600">' + esc(s.weeklyKeyword) + '</span></div>';
    }
    html += '</div>';
  }
  ...
  if (s.unified && s.running) {
    ...
    for (let pi = 0; pi < phases.length; pi++) {
      const pct = pi < curIdx ? '100%' : (pi === curIdx ? (s.phase === 'generating_days' &&
      s.current >= 0 ? Math.round(((s.current + 0.5) / 5) * 100) + '%' : '50%') : '0%');
      ...
      margin-bottom:4px"><div style="height:100%;background:' + bg + ';width:' + pct + ';
      transition:width 0.3s"></div></div>';
      html += '<div style="font-size:10px;
      color:' + (pi === curIdx ? 'var(--text)' : 'var(--dim)') + ';
      font-weight:' + (pi === curIdx ? '600' : '400') + '">' + phaseLabels[pi] + '</div>';
      html += '</div>';
    }
    html += '</div>';
  }
  ...
  if (s.unified && s.running && s.phase === 'planning' && s.plannerStepStatus) {
    ...
    html += '<div style="font-size:12px;font-weight:600;color:var(--accent2);
    margin-bottom:8px">Weekly Planner - Agent Steps</div>';
    for (const [agent, st] of Object.entries(s.plannerStepStatus)) {
      ...
      font-size:12px">';
      html += '<span style="color:' + (st === 'running' ? 'var(--text)' : 'var(--dim)')
      + '">' + label + '</span>';
      html += '<span style="color:' + stColor + ';font-weight:600;
      font-size:11px">' + icon + '</span>';
      html += '</div>';
    }
    html += '</div>';
  }
  ...
  for (let i = 0; i < 5; i++) {
    ...
    else if (r?.status === 'failed') { bg = 'var(--red)'; color = '#fff';
    label = DAY_NAMES[i] + ' Failed'; }
    else if (s.running && i === s.current) { bg = 'var(--blue)'; color = '#fff';
    label = DAY_NAMES[i] + '...'; }
    else { bg = 'var(--border)'; color = 'var(--dim)'; label = DAY_NAMES[i]; }
    html += '<div style="padding:6px 14px;border-radius:6px;font-size:12px;font-weight:600;
  ...
  if (s.running && cur?.stepStatus && Object.keys(cur.stepStatus).length > 0) {
    ...
    padding:12px;margin-bottom:8px">';
    html += '<div style="font-size:12px;font-weight:600;color:var(--accent2);
    margin-bottom:8px">' + DAY_NAMES[s.current] + ' - Pipeline Steps</div>';
    const entries = Object.entries(steps);
    const total = entries.length;
    const completed = entries.filter(([,v]) => v === 'completed' || v === 'skipped').length;
    html += '<div style="height:4px;background:var(--border);border-radius:2px;overflow:hidden;
    margin-bottom:10px">';
    html += '<div style="height:100%;background:var(--green);
    width:' + Math.round((completed/total)*100) + '%;transition:width 0.3s"></div>';
    html += '</div>';
    for (const [agent, st] of entries) {
      ...
      html += '<span style="color:' + (st === 'running' ? 'var(--text)' : 'var(--dim)') + ';
      font-weight:' + (st === 'running' ? '600' : '400') + '">';
      if (st === 'running') html += '<span style="display:inline-block;width:6px;height:6px;
      border-radius:50%;background:var(--blue);margin-right:6px"></span>';
      html += label + '</span>';
      html += '<span style="color:' + stColor + ';font-weight:600;
      font-size:11px">' + icon + '</span>';
      html += '</div>';
  ...
  if (!s.running) {
    ...
    if (s.phase === 'failed' || s.phase === 'completed') {
      ...
      color:' + (s.phase === 'failed' ? 'var(--red)' : 'var(--green)') + ';
      font-weight:600">
      ' + (s.phase === 'failed' ? 'Failed: ' + (s.errorMsg || s.phaseDetail || 'Unknown error')
      : done + '/5 completed + guide built') + '</span>';
    } else {
      html += '<span style="font-size:13px;color:var(--green);
      font-weight:600">' + done + '/5 completed</span>';
      if (fail) html += '<span style="font-size:13px;color:var(--red)">' + fail + ' failed</span>';
    }
    if (s.totalCost) html += '<span style="font-size:13px;
    color:var(--accent2)">Total: $' + s.totalCost.toFixed(2) + '</span>';
    for (let fi = 0; fi < 5; fi++) {
      if (s.results[fi]?.status === 'failed') {
        html += '<button class="btn btn-red" style="font-size:11px;
        padding:4px 10px" onclick="retryOneDay(' + fi + ')">Retry ' + DAY_NAMES[fi] + '</button>';
      }
    }
    html += '<button class="btn btn-dim" style="margin-left:auto;
    font-size:12px" onclick="genAllState=null;saveGenAllState();
    renderGenAllProgress()">Dismiss</button>';
    html += '</div>';
    // Show error details for failed days
    for (let fi = 0; fi < 5; fi++) {
      if (s.results[fi]?.status === 'failed' && s.results[fi]?.errorMsg) {
        html += '<div style="margin-top:6px;padding:8px 12px;background:#2d0000;
        border:1px solid var(--red);border-radius:6px;font-size:12px">';
        html += '<span style="color:var(--red);
        font-weight:600">' + DAY_NAMES[fi] + ':</span> <span style="color:#fecaca">
        ' + esc(s.results[fi].errorMsg.substring(0, 200)) + '</span>';
        html += '</div>';
      }
    }
  }
  ...
  if (btn) {
    ...
    if (s.unified && s.running) {
      ...
      btn.textContent = 'Day ' + ((s.current >= 0 ? s.current : 0) + 1) + '/5...';
      else if (s.phase === 'building_guide') btn.textContent = 'Building guide...';
      else btn.textContent = 'Starting...';
    } else if (s.running) {
      const curAgent = cur?.stepStatus ? Object.entries(cur.stepStatus).find(([,
      v]) => v === 'running') : null;
      const agentLabel = curAgent ? (AGENT_LABELS[curAgent[0]] || curAgent[0]) : '';
      btn.textContent = agentLabel ? agentLabel + '... (' + (s.current+1) + '/5)
      ' : 'Generating ' + DAY_NAMES[s.current] + '... (' + (s.current+1) + '/5)';
    } else {
...
async function resumeUnifiedGenAll() {
  if (!genAllState?.weekId) { genAllState.running = false; saveGenAllState();
  renderGenAllProgress(); return; }
  let done = false;
  let failCount = 0;
  while (!done) {
    await new Promise(ok => setTimeout(ok, 2500));
    try {
      ...
      if (st.day_run_ids) {
        for (let i = 0; i < st.day_run_ids.length; i++) {
          if (!genAllState.results[i]) genAllState.results[i] = { day: DAY_NAMES[i], status: 'done',
          runId: st.day_run_ids[i] };
        }
      }
      done = st.status === 'completed' || st.status === 'failed';
      if (st.status === 'failed') genAllState.errorMsg = st.error || 'Unknown error';
  ...
  while (!done) {
    ...
    try {
      ...
      if (done) {
        ...
        toast(DAY_NAMES[genAllState.current] + (hasFail ? ' finished with errors' : ' done!'),
        !hasFail);
      }
...
async function runOneDay(i) {
  ...
  genAllState.results[i] = { day: DAY_NAMES[i], status: 'running', stepStatus: {},
  startTime: Date.now() };
  saveGenAllState();
  renderGenAllProgress();
  try {
      ...
      try {
        ...
        if (done) {
          ...
          if (hasFail && status.step_errors) {
            const errParts = Object.entries(status.step_errors).map(([k,v]) => k.replace(/_/g,
            ' ') + ': ' + v);
            genAllState.results[i].errorMsg = errParts.join('; ');
          }
          toast(DAY_NAMES[i] + (hasFail ? ' finished with errors' : ' done!'), !hasFail);
        }
      } catch (pollErr) { done = true; genAllState.results[i].status = 'failed';
      genAllState.results[i].errorMsg = pollErr.message || 'Connection lost'; }
      saveGenAllState();
      renderGenAllProgress();
    }
  } catch (e) {
    genAllState.results[i] = { day: DAY_NAMES[i], status: 'failed',
    errorMsg: e.message || 'Pipeline start failed' };
...
async function generateAllDays() {
  genAllState = { running: true, current: -1, total: 5, startTime: Date.now(), results: [],
  totalCost: 0, unified: true, weekId: null, phase: 'starting',
  phaseDetail: 'Initializing unified weekly generation...', weeklyTheme: null, giftTheme: null,
  weeklyKeyword: null };
  saveGenAllState();
  renderGenAllProgress();
  try {
    ...
    while (!done) {
      ...
      try {
        ...
        if (st.day_run_ids) {
          for (let i = 0; i < st.day_run_ids.length; i++) {
            if (!genAllState.results[i]) genAllState.results[i] = { day: DAY_NAMES[i],
            status: 'done', runId: st.day_run_ids[i] };
          }
        }
...
async function retryOneDay(dayIndex) {
  ...
  genAllState.results[dayIndex] = { day: DAY_NAMES[dayIndex], status: 'running', stepStatus: {},
  startTime: Date.now() };
  saveGenAllState();
  renderGenAllProgress();
    ...
    <div class="section">
      ...
      <div class="card" style="margin-bottom:16px">
        <div style="display:flex;gap:12px;align-items:end;flex-wrap:wrap">
          <div>
            <label style="font-size:12px;color:var(--dim);display:block;
            margin-bottom:4px">Workflow</label>
            ...
            <div id="wf-desc" style="font-size:12px;color:var(--text);margin-top:8px;
            max-width:500px;line-height:1.6;opacity:0.85"></div>
          </div>
          <div>
            <label style="font-size:12px;color:var(--dim);display:block;
            margin-bottom:4px">Day of Week</label>
      ...
      <div style="margin-top:12px;display:flex;align-items:center;gap:8px">
        <label style="font-size:12px;color:var(--dim);cursor:pointer;display:flex;
        align-items:center;gap:6px">
          <input type="checkbox" id="verbose-toggle" ${verboseMode ? 'checked' : ''}
          onchange="verboseMode=this.checked;localStorage.setItem('tce_verbose',verboseMode);
          if(activePipelineRun)pollPipeline()">
...
async function runPipeline() {
  ...
  try {
    const r = await api('/pipeline/run', { method: 'POST', body: JSON.stringify({ workflow: wf,
...
function formatLogLine(raw) {
  ...
  if (!m) return '<div style="padding:4px 0 4px 8px;color:var(--text);font-size:13px;
  line-height:1.5">' + esc(raw) + '</div>';
  const time = m[1];
  let msg = m[2];
  ...
  return '<div style="display:flex;gap:8px;padding:3px 0 3px 8px;font-size:13px;
  line-height:1.6"><span style="color:var(--dim);font-size:11px;min-width:52px;
  font-family:monospace">' + time + '</span><span style="color:' + color + '">' + icon + esc(msg)
  + '</span></div>';
}
...
async function pollPipeline() {
  ...
  try {
    ...
    if (!pipelineInitialized || !container.querySelector('.pipeline-card')) {
      ...
      let html = '<div class="card pipeline-card"><h3 style="margin-bottom:12px">
      Pipeline: ' + activePipelineRun.substring(0, 8) + '...</h3>';
      // Step badges row
      html += '<div class="pipeline-steps" id="pipeline-badges">';
      for (const [step, status] of Object.entries(statuses)) {
        html += '<div class="step-badge ' + status + '" id="badge-' + step + '"
        style="cursor:pointer" onclick="toggleStep(\\'' + step + '\\')">' + step.replace(/_/g,
        ' ') + '</div>';
      }
      html += '</div>';
      ...
      if (verboseMode) {
        ...
        for (const [step, status] of Object.entries(statuses)) {
          ...
          const statusColor = status === 'completed' ? 'var(--green)
          ' : status === 'running' ? 'var(--blue)' : status === 'failed' ? 'var(--red)
          ' : 'var(--dim)';
          html += '<div style="border:1px solid var(--border);border-radius:8px;margin-bottom:8px;
          overflow:hidden" id="step-section-' + step + '">';
          html += '<div style="display:flex;justify-content:space-between;align-items:center;
          padding:10px 14px;cursor:pointer;background:var(--card);
          user-select:none" onclick="toggleStep(\\'' + step + '\\')">';
          html += '<div style="display:flex;align-items:center;
          gap:8px"><span id="step-arrow-' + step + '" style="color:var(--dim);
          font-size:12px">' + (isExpanded ? '▾' : '▸') + '</span>';
          html += '<span style="font-weight:600;
          font-size:13px">' + step.replace(/_/g, ' ') + '</span>';
          html += '<span style="font-size:11px;color:' + statusColor + ';
          font-weight:500" id="step-status-' + step + '">' + status + '</span></div>';
          html += '<span style="font-size:11px;
          color:var(--dim)" id="step-summary-' + step + '"></span>';
          html += '</div>';
          html += '<div id="step-logs-' + step + '" style="display:' + (isExpanded ? 'block' :
          'none') + ';max-height:350px;overflow-y:auto;padding:4px 6px;background:#0d0f14;
    ...
    if (errEl) {
      ...
      if (errEntries.length) {
        ...
        for (const [step, err] of errEntries) eh += '<div style="color:var(--red);
        font-size:13px"><strong>' + step.replace(/_/g, ' ') + ':</strong> ' + esc(err).substring(0,
        300) + '</div>';
        eh += '</div>';
        errEl.innerHTML = eh;
      }
    ...
    if (footerEl && allDone) {
      ...
      let fh = '<div style="margin-top:12px;display:flex;align-items:center;gap:12px;
      flex-wrap:wrap">';
      fh += '<span style="color:' + (hasFailed ? 'var(--yellow)' : 'var(--green)') + ';
      font-weight:600">' + (hasFailed ? 'Pipeline finished with errors' : 'Pipeline complete')
      + '</span>';
      ...
      if (statuses.docx_guide_builder === 'completed') {
        fh += '<button class="btn btn-green" style="font-size:14px;
        padding:10px 20px" onclick="downloadLatestGuide()">Download Guide</button>';
      }
      if (hasCompleted) {
        fh += '<button class="btn btn-dim" style="font-size:13px;
        padding:8px 16px" onclick="currentTab=\\'packages\\';
        document.querySelectorAll(\\'.nav button\\').forEach(b=>b.classList.toggle(\\'active\\',
        b.dataset.tab===\\'packages\\'));render()">View Packages</button>';
      }
      fh += '</div>';
...
async function downloadLatestGuide() {
  try {
    ...
    if (!guides.length)
    { alert('No guides found. The guide builder may not have saved to the database.'); return; }
...
async function renderPackages() {
  const app = document.getElementById('app');
  app.innerHTML = '<div class="section"><div style="display:flex;justify-content:space-between;
  align-items:center;margin-bottom:16px"><h2>Content Packages</h2><label style="font-size:12px;
  color:var(--dim);cursor:pointer;display:flex;align-items:center;
  gap:6px"><input type="checkbox" id="show-archived" ' + (showArchived ? 'checked' : '')
  + ' onchange="showArchived=this.checked;
  renderPackages()
  "> Show archived</label></div><div id="pkg-list"><div class="empty">Loading...</div></div></div>';
  try {
    const pkgs = await api('/content/packages' + (showArchived ? '?include_archived=true' : ''));
    if (!pkgs.length) { document.getElementById('pkg-list')
    .innerHTML = '<div class="empty">No packages yet. Run a pipeline first.</div>'; return; }
    let html = '<div class="packages-list">';
    for (const p of pkgs) {
      const statusTag = p.approval_status === 'approved' ? 'tag-approved' : p.approval_status ===
      'rejected' ? 'tag-rejected' : 'tag-draft';
      html += '<div class="pkg-card" id="pkg-' + p.id + '"' + (p.is_archived ? '
      style="opacity:0.5"' : '') + '>';
      html += '<div class="pkg-header"><span class="tag ' + statusTag + '">
      ' + p.approval_status + '</span>';
      if (p.is_archived) html += '<span style="font-size:11px;color:var(--dim);
      background:var(--border);padding:2px 8px;border-radius:4px">ARCHIVED</span>';
      html += '<span style="font-size:12px;
      color:var(--dim)">' + new Date(p.created_at).toLocaleString() + '</span></div>';
      ...
      if (fbWc) html += '<span style="color:var(--accent2);
      font-weight:600">FB: ' + fbWc + ' words</span>';
      if (liWc) html += '<span style="color:var(--accent2);
      font-weight:600">LI: ' + liWc + ' words</span>';
      if (p.pipeline_run_id) html += '<span>Run: ' + p.pipeline_run_id.substring(0, 8) + '</span>';
      html += '</div>';
      // Tabs
      html += '<div class="tabs">';
      html += '<button class="active" onclick="showPostTab(this,
      \\'fb-' + pid + '\\')">Facebook</button>';
      html += '<button onclick="showPostTab(this,\\'li-' + pid + '\\')">LinkedIn</button>';
      if (p.hook_variants?.length) html += '<button onclick="showPostTab(this,
      \\'hooks-' + pid + '\\')">Hooks (' + p.hook_variants.length + ')</button>';
      if (p.quality_scores) html += '<button onclick="showPostTab(this,
      \\'qa-' + pid + '\\')">QA Scores</button>';
      if (p.dm_flow) html += '<button onclick="showPostTab(this,
      \\'dm-' + pid + '\\')">DM Flow</button>';
      if (p.image_prompts?.length) html += '<button onclick="showPostTab(this,
      \\'img-' + pid + '\\')">Images (' + p.image_prompts.length + ')</button>';
      html += '</div>';
      html += '<div id="fb-' + pid + '"><div class="post-preview">
      ' + esc(fbText || 'No Facebook post generated') + '</div></div>';
      html += '<div id="li-' + pid + '" style="display:none"><div class="post-preview">
      ' + esc(liText || 'No LinkedIn post generated') + '</div></div>';
      ...
      if (p.quality_scores) {
        ...
        if (composite != null) {
          ...
          const compColor = compVal >= 7 ? 'var(--green)' : compVal >= 5 ? 'var(--yellow)
          ' : 'var(--red)';
          const compLabel = compVal >= 7 ? 'PASS' : compVal >= 5 ? 'CONDITIONAL' : 'FAIL';
          html += '<div style="display:flex;align-items:center;gap:12px;margin-bottom:12px;
          padding:12px;background:#111318;border-radius:8px">';
          html += '<div style="font-size:32px;font-weight:800;
          color:' + compColor + '">' + (typeof compVal === 'number' ? compVal.toFixed(1) : compVal)
          + '</div>';
          html += '<div><div style="font-size:14px;font-weight:700;
          color:' + compColor + '">' + compLabel + '</div><div style="font-size:12px;
          color:var(--dim)">Composite QA Score</div></div>';
          html += '</div>';
        }
        ...
        for (const [k, v] of Object.entries(p.quality_scores)) {
          ...
          html += '<div class="qa-item" ' + (justification ? 'title="' + esc(justification)
          + '" style="cursor:help"' : '') + '>';
          html += '<div class="label">' + icon + ' ' + k.replace(/_/g, ' ') + '</div>';
          html += '<div class="score" style="color:' + color + '">
          ' + (typeof score === 'number' ? score.toFixed(1) : score) + '</div>';
          html += '</div>';
        }
        html += '</div></div>';
      }
      if (p.dm_flow) {
        ...
        if (dm.trigger) {
          html += '<div style="background:#1e1b4b;border:1px solid var(--accent);border-radius:8px;
          padding:14px">';
          html += '<div style="font-size:11px;color:var(--accent2);text-transform:uppercase;
          letter-spacing:.5px;margin-bottom:6px">Trigger Keyword</div>';
          html += '<div style="font-size:20px;font-weight:700;
          color:var(--accent2)">' + esc(dm.trigger) + '</div>';
          html += '</div>';
        }
        if (dm.ack_message) {
          html += '<div style="background:#111318;border:1px solid var(--border);border-radius:8px;
          padding:14px">';
          html += '<div style="font-size:11px;color:var(--green);text-transform:uppercase;
          letter-spacing:.5px;margin-bottom:6px">Instant Reply (when they comment)</div>';
          html += '<div style="font-size:14px;line-height:1.6;
          white-space:pre-wrap">' + esc(dm.ack_message) + '</div>';
          html += '</div>';
        }
        if (dm.delivery_message) {
          html += '<div style="background:#111318;border:1px solid var(--border);border-radius:8px;
          padding:14px">';
          html += '<div style="font-size:11px;color:var(--blue);text-transform:uppercase;
          letter-spacing:.5px;margin-bottom:6px">Delivery Message (with the guide)</div>';
          html += '<div style="font-size:14px;line-height:1.6;
          white-space:pre-wrap">' + esc(dm.delivery_message) + '</div>';
          html += '</div>';
        }
        if (dm.follow_up) {
          html += '<div style="background:#111318;border:1px solid var(--border);border-radius:8px;
          padding:14px">';
          html += '<div style="font-size:11px;color:var(--yellow);text-transform:uppercase;
          letter-spacing:.5px;margin-bottom:6px">Follow-up (24-48h later)</div>';
          html += '<div style="font-size:14px;line-height:1.6;
          white-space:pre-wrap">' + esc(dm.follow_up) + '</div>';
          html += '</div>';
        }
        ...
        for (const [k,v] of Object.entries(dm)) {
          if (!knownKeys.has(k) && v) {
            html += '<div style="background:#111318;border:1px solid var(--border);
            border-radius:8px;padding:14px">';
            html += '<div style="font-size:11px;color:var(--dim);text-transform:uppercase;
            letter-spacing:.5px;margin-bottom:6px">' + esc(k.replace(/_/g, ' ')) + '</div>';
            html += '<div style="font-size:14px;line-height:1.6;
            white-space:pre-wrap">' + esc(typeof v === 'string' ? v : JSON.stringify(v, null,
            2)) + '</div>';
            html += '</div>';
          }
        }
        html += '</div>';
        html += '<button class="btn btn-dim" style="margin-top:12px;
        font-size:12px" onclick="copyDmFlow(this,
        \\'' + pid + '\\')">Copy All DM Messages</button>';
        html += '</div>';
      }
      if (p.image_prompts?.length) {
        ...
        if (!hasAnyImages) {
          html += '<button class="btn btn-blue" id="gen-img-btn-' + pid + '"
          onclick="generateImages(\\'' + p.id + '\\', this)">Generate Images with AI</button>';
          html += '<span style="font-size:12px;
          color:var(--dim)">Uses fal.ai Flux Pro (~$0.03/image,
          ' + p.image_prompts.length + ' images)</span>';
        } else {
          html += '<button class="btn btn-dim" id="gen-img-btn-' + pid + '"
          onclick="generateImages(\\'' + p.id + '\\', this)">Regenerate Images</button>';
        ...
        // Image grid
        html += '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(320px,1fr));
        gap:12px">';
        for (const ip of p.image_prompts) {
          const promptText = ip.prompt_text || ip.detailed_prompt || '';
          const bestPlat = ip.best_platform || 'fal_ai';
          const platColors = { fal_ai: '#16a34a', midjourney: '#a855f7', gemini: '#3b82f6',
          dall_e: '#f97316' };
          const platLabels = { fal_ai: 'fal.ai', midjourney: 'Midjourney', gemini: 'Gemini',
          dall_e: 'DALL-E' };
          ...
          html += '<div style="font-weight:600;font-size:14px;color:var(--accent2);
          flex:1">' + esc(ip.prompt_name || ip.visual_job || 'Image') + '</div>';
          html += '<span style="padding:3px 10px;border-radius:12px;font-size:11px;font-weight:600;
          background:' + platColor + '22;color:' + platColor + ';
          border:1px solid ' + platColor + '44" title="' + esc(ip.best_platform_reason || '')
          + '">' + platLabel + '</span>';
          html += '</div>';
          // Platform hint message if not fal_ai
          if (bestPlat !== 'fal_ai' && ip.best_platform_reason) {
            html += '<div style="font-size:12px;color:' + platColor + ';margin-bottom:8px;
            padding:6px 10px;background:' + platColor + '11;
            border-radius:6px">Try on ' + platLabel + ' - ' + esc(ip.best_platform_reason)
            + '</div>';
          }
          // Show image first if available
          if (ip.image_url) {
            html += '<div style="margin-bottom:12px;border-radius:8px;overflow:hidden;
            border:1px solid var(--border)">';
            html += '<img src="' + esc(ip.image_url) + '" style="width:100%;
            display:block" loading="lazy">';
            html += '</div>';
          }
          if (ip.visual_intent) html += '<div style="font-size:12px;color:var(--text);
          margin-bottom:8px;font-style:italic">' + esc(ip.visual_intent) + '</div>';
          html += '<div style="font-size:12px;color:var(--dim);margin-bottom:8px;display:flex;
          gap:12px;flex-wrap:wrap">';
          if (ip.mood) html += '<span>Mood: <strong>' + esc(ip.mood) + '</strong></span>';
          if (ip.aspect_ratio) html += '<span>Ratio: <strong>' + esc(ip.aspect_ratio)
          + '</strong></span>';
          if (ip.platform_fit) html += '<span>Platform: <strong>' + esc(ip.platform_fit)
          + '</strong></span>';
          if (ip.color_logic) html += '<span>Colors: ' + esc(String(ip.color_logic).slice(0,
          60)) + '</span>';
          html += '</div>';
          // Prompt text always visible
          html += '<div style="font-size:12px;
          margin-top:8px"><strong style="color:var(--green)">Prompt:</strong></div>';
          html += '<div class="post-preview" style="font-size:12px;margin:4px 0 0;max-height:200px;
          white-space:pre-wrap;overflow-y:auto">' + esc(promptText) + '</div>';
          if (ip.negative_prompt) {
            html += '<div style="font-size:12px;
            margin-top:8px"><strong style="color:var(--red)">Negative:</strong></div>';
            html += '<div style="font-size:11px;color:var(--dim);
            margin-top:4px">' + esc(ip.negative_prompt) + '</div>';
          }
          if (ip.rationale) html += '<div style="font-size:11px;color:var(--dim);margin-top:8px;
          border-top:1px solid var(--border);
          padding-top:8px">Rationale: ' + esc(ip.rationale) + '</div>';
          // Copy buttons
          html += '<div style="display:flex;gap:6px;margin-top:8px;flex-wrap:wrap">';
          html += '<button class="btn btn-dim" style="font-size:11px" onclick="copyImagePrompt(this)
          ">Copy Prompt</button>';
          if (bestPlat !== 'fal_ai') {
            html += '<button class="btn btn-dim" style="font-size:11px;
            border-color:' + platColor + '44;
            color:' + platColor + '" onclick="copyForPlatform(this,
            \\'' + platLabel + '\\')">Copy for ' + platLabel + '</button>';
          }
          html += '</div>';
          html += '</div>';
        }
      ...
      if (p.approval_status === 'draft') {
        html += '<button class="btn btn-green" onclick="approvePackage(\\'' + p.id + '\\')
        ">Approve</button>';
        html += '<button class="btn btn-red" onclick="rejectPackage(\\'' + p.id + '\\')
        ">Reject</button>';
      } else if (p.approval_status === 'approved') {
        html += '<span style="color:var(--green);font-weight:600;font-size:13px;
        padding:8px 0">Approved</span>';
      } else if (p.approval_status === 'rejected') {
        html += '<span style="color:var(--red);font-weight:600;font-size:13px;
        padding:8px 0">Rejected</span>';
        html += '<button class="btn btn-dim" onclick="resetPackageStatus(\\'' + p.id + '\\')
        ">Reset to Draft</button>';
      }
      html += '<button class="btn btn-blue" onclick="exportPackage(\\'' + p.id + '\\')
      ">Export</button>';
      html += '<button class="btn btn-dim" onclick="showFeedbackForm(\\'' + p.id + '\\',
      this)">Feedback</button>';
      html += '<button class="btn btn-dim" style="border-color:var(--accent)
      " onclick="showRevisedCopyForm(\\'' + p.id + '\\', this)">Edit & Submit Copy</button>';
      html += '<button class="btn btn-dim" onclick="copyPost(\\'' + pid + '\\', this,
      \\'Facebook post\\')">Copy FB Post</button>';
      html += '<button class="btn btn-dim" onclick="copyPost(\\'li-' + pid + '\\', this,
      \\'LinkedIn post\\')">Copy LI Post</button>';
      if (p.is_archived) {
        html += '<button class="btn btn-dim" onclick="unarchivePackage(\\'' + p.id + '\\')
        ">Unarchive</button>';
      } else {
        html += '<button class="btn btn-dim" onclick="archivePackage(\\'' + p.id + '\\')
        ">Archive</button>';
      }
      html += '</div>';
      html += '</div>';
  ...
  } catch (e) { document.getElementById('pkg-list')
  .innerHTML = '<div class="empty">Error: ' + e.message + '</div>'; }
}
...
function showPostTab(btn, id) {
  ...
  card.querySelectorAll('[id^="fb-"],[id^="li-"],[id^="hooks-"],[id^="qa-"],[id^="img-"],
  [id^="dm-"]').forEach(el => {
...
async function resetPackageStatus(id) {
  try {
    await api('/content/packages/' + id, { method: 'PATCH',
    body: JSON.stringify({ approval_status: 'draft' }) });
...
async function exportPackage(id) {
  const result = await api('/content/packages/' + id + '/export?platform=manual',
  { method: 'POST' });
  const w = window.open('', '_blank');
  w.document.write('<pre style="font-family:monospace;white-space:pre-wrap;padding:20px;
  max-width:800px;margin:auto">' + JSON.stringify(result, null, 2) + '</pre>');
}
...
function copyDmFlow(btn, pid) {
  ...
  btn.style.color = '#000';
  setTimeout(() => { btn.textContent = 'Copy All DM Messages'; btn.style.background = '';
...
function copyImagePrompt(btn) {
  ...
  if (preview) {
    ...
    btn.style.background = 'var(--green)';
    btn.style.color = '#000';
    setTimeout(() => { btn.textContent = 'Copy Prompt'; btn.style.background = '';
    btn.style.color = ''; }, 2000);
...
function copyForPlatform(btn, platformName) {
  ...
  if (preview) {
    ...
    btn.style.background = 'var(--green)';
    btn.style.color = '#000';
    setTimeout(() => { btn.textContent = orig; btn.style.background = ''; btn.style.color = '';
    }, 2000);
...
async function generateImages(packageId, btn) {
  ...
  const progressEl = document.getElementById('gen-img-progress-' + pid)
  || btn.parentElement.nextElementSibling;
  if (progressEl) {
    progressEl.innerHTML = '<div style="background:#1e1b4b;border:1px solid var(--accent);
    border-radius:8px;padding:14px;margin-bottom:12px">' +
      '<div style="font-size:12px;color:var(--accent2);
      margin-bottom:8px">Generating images with fal.ai Flux Pro...</div>' +
      '<div style="height:4px;background:var(--border);border-radius:2px;overflow:hidden">' +
      '<div style="height:100%;background:var(--accent2);width:30%;border-radius:2px"></div>' +
      '</div>' +
      '<div style="font-size:11px;color:var(--dim);
      margin-top:6px">This may take 15-30 seconds per image...</div>' +
      '</div>';
  }

  try {
    const resp = await fetch('/api/v1/content/packages/' + packageId + '/generate-images',
    { method: 'POST' });
    if (!resp.ok) {
    ...
    setTimeout(async () => {
      ...
      setTimeout(() => {
        ...
        if (imgTab) {
          ...
          if (card) {
            const imgBtn = Array.from(card.querySelectorAll('.tabs button'))
            .find(b => b.textContent.startsWith('Images'));
...
function copyPost(pid, btn, label) {
  ...
  const el = document.getElementById(pid.startsWith('fb-') || pid.startsWith('li-')
  ? pid : 'fb-' + pid);
  if (el) {
...
async function renderCorpus() {
  ...
  app.innerHTML = `
    <div class="section">
      ...
      <div class="upload-zone" id="upload-zone" onclick="document.getElementById('file-input')
      .click()">
        <input type="file" id="file-input" accept=".docx,
        .txt" style="display:none" onchange="uploadFile(this)">
        <div style="font-size:24px">+</div>
        <p>Click to upload DOCX or TXT file</p>
        <p style="font-size:11px;
        margin-top:4px">Auto-analyzes corpus (extracts examples, scores engagement,
        mines patterns)</p>
      </div>
  ...
  try {
    ...
    if (!docs.length) { document.getElementById('docs-list')
    .innerHTML = '<div class="empty">No documents uploaded yet.</div>'; return; }
    let html = '<div class="docs-list">';
    for (const d of docs) {
      html += '<div class="doc-row">';
      html += '<div class="doc-info"><div class="doc-name">' + esc(d.file_name) + '</div>';
      html += '<div class="doc-meta">' + d.file_type + ' - ' + (d.pages || '?')
      + ' pages - ' + new Date(d.created_at).toLocaleDateString() + '</div></div>';
      html += '<button class="btn btn-dim" onclick="viewExamples(\\'' + d.id + '\\',
      \\'' + esc(d.file_name) + '\\')">View Examples</button>';
      html += '</div>';
    }
    html += '</div>';
    document.getElementById('docs-list').innerHTML = html;
  } catch (e) { document.getElementById('docs-list')
  .innerHTML = '<div class="empty">Error: ' + e.message + '</div>'; }
}
...
async function uploadFile(input) {
  ...
  try {
    const r = await fetch(API + '/documents/upload?auto_analyze=true', { method: 'POST',
    body: fd });
    if (!r.ok) {
      const err = await r.text();
    ...
    .innerHTML = '<input type="file" id="file-input" accept=".docx,
    .txt" style="display:none" onchange="uploadFile(this)
    "><div style="font-size:24px">+</div><p>Click to upload DOCX or TXT file</p>
    <p style="font-size:11px;margin-top:4px">Auto-analyzes corpus</p>';
    renderCorpus();
  }
}
...
async function viewExamples(docId, name) {
  try {
    ...
    let creatorMap = {};
    try { const creators = await api('/profiles/creators');
    for (const c of creators) creatorMap[c.id] = c; } catch(e) {}
    const w = window.open('', '_blank');
    let h = '<html><head><style>body{font-family:-apple-system,sans-serif;padding:20px;
    max-width:1000px;margin:auto;background:#0f1117;color:#e4e4e7}h1{font-size:20px;
    margin-bottom:16px}.ex{border:1px solid #2a2d3a;border-radius:10px;padding:20px;margin:16px 0;
    background:#1a1d27}.meta{display:flex;gap:12px;flex-wrap:wrap;margin-bottom:12px;font-size:12px;
    color:#71717a}.badge{background:#2a2d3a;padding:2px 8px;border-radius:4px;
    font-size:11px}.score-box{display:inline-block;background:#6366f1;color:#fff;padding:4px 10px;
    border-radius:6px;font-weight:700;font-size:14px;margin-right:8px}.post-text{background:#0f1117;
    border:1px solid #2a2d3a;border-radius:6px;padding:12px;margin:8px 0;white-space:pre-wrap;
    font-size:13px;line-height:1.6;direction:rtl;text-align:right;max-height:200px;
    overflow-y:auto}.section-label{font-size:11px;color:#6366f1;text-transform:uppercase;
    letter-spacing:.5px;margin-top:12px;margin-bottom:4px}.tags{display:flex;gap:4px;flex-wrap:wrap;
    margin:4px 0}.tag{background:#1e3a5f;color:#60a5fa;padding:2px 6px;border-radius:3px;
    font-size:11px}.tag.topic{background:#1e3a2f;color:#4ade80}.engagement{display:flex;gap:16px;
    margin-top:8px;font-size:12px;color:#71717a}.expand-btn{background:none;
    border:1px solid #2a2d3a;color:#818cf8;padding:4px 10px;border-radius:4px;cursor:pointer;
    font-size:11px;margin-top:6px}.inspire-btn{background:linear-gradient(135deg,#6366f1,#a855f7);
    color:#fff;border:none;padding:8px 16px;border-radius:6px;cursor:pointer;font-size:12px;
    font-weight:600;margin-top:10px;
    transition:opacity .2s}.inspire-btn:hover{opacity:.85}.inspire-btn:disabled{opacity:.5;
    cursor:not-allowed}.inspire-status{font-size:11px;color:#a78bfa;margin-top:6px;
    min-height:16px}</style></head><body>';
  ...
  try {
    ...
    const poll = setInterval(async () => {
      try {
        ...
        if (running.length) status.textContent = 'Running: ' + running.join(',
        ') + ' (' + done + '/' + total + ' done)';
        if (st.status === 'completed') {
          clearInterval(poll);
          ...
          status.textContent = 'Pipeline completed. All agents finished. Click button to refresh
          dashboard.';
...
<\\/script>`;
    ...
    for (let i = 0; i < examples.length; i++) {
      ...
      if (ex.final_score != null) h += '<div class="score-box">' + ex.final_score.toFixed(2)
      + '</div>';
      else if (ex.raw_score != null)
      h += '<div class="score-box" style="background:#71717a">Raw: ' + ex.raw_score.toFixed(2)
      + '</div>';
      h += '</div>';
      // Word count
      const wordCount = ex.post_text_raw ? ex.post_text_raw.trim().split(/\\s+/).length : 0;
      if (wordCount > 0) h += '<div style="font-size:11px;color:#71717a;
      margin-bottom:8px">' + wordCount + ' words</div>';
      ...
      if (ex.hook_text) {
        ...
        h += '<div style="font-size:13px;direction:rtl;text-align:right;padding:6px 0;
        border-bottom:1px solid #2a2d3a">' + ex.hook_text + '</div>';
      }
      ...
      if (ex.cta_text) {
        ...
        h += '<div style="font-size:13px;direction:rtl;text-align:right;padding:6px 0;
        color:#eab308">' + ex.cta_text + '</div>';
      }
      // Tags
      if (ex.tone_tags?.length || ex.topic_tags?.length) {
        h += '<div style="margin-top:8px">';
        if (ex.tone_tags?.length) { h += '<div class="tags">';
        for (const t of ex.tone_tags) h += '<span class="tag">' + t + '</span>'; h += '</div>'; }
        if (ex.topic_tags?.length) { h += '<div class="tags">';
        for (const t of ex.topic_tags) h += '<span class="tag topic">' + t + '</span>';
        h += '</div>'; }
        h += '</div>';
      }
      // Engagement data
      ...
      if (ex.post_text_raw) {
        ...
        h += '<button class="inspire-btn" id="inspire-btn-' + i + '" onclick="inspireFromExample('
        + i + ')">Inspire Post</button>';
        h += '<div class="inspire-status" id="inspire-status-' + i + '">
        Generate a new post inspired by this creator\\\'s style</div>';
        h += '</div>';
      }
      h += '</div>';
    }
...
async function renderVoice() {
  const app = document.getElementById('app');
  app.innerHTML = '<div class="section"><h2>Founder Voice Profiles</h2><div id="voice-list">
  <div class="empty">Loading...</div></div></div>';
  try {
    const profiles = await api('/profiles/founder-voice');
    if (!profiles.length) { document.getElementById('voice-list')
    .innerHTML = '<div class="empty">
    No voice profiles yet. Extract from a book via Corpus tab.</div>'; return; }
    let html = '';
    for (const p of profiles) {
      html += '<div class="voice-profile">';
      html += '<h4>Profile from: ' + (p.source_document_ids?.join(', ') || 'unknown').substring(0,
      36) + '...</h4>';
      // Tone range bars
      if (p.tone_range) {
        html += '<div style="margin:8px 0">';
        for (const [k, v] of Object.entries(p.tone_range)) {
          html += '<div class="tone-bar"><span class="name">' + k + '</span>
          <div class="bar" style="width:' + (v * 10) + '%"></div><span>' + v + '/10</span></div>';
        }
        html += '</div>';
      }
      if (p.humor_type) html += '<div style="font-size:13px;
      margin:4px 0">Humor: <strong>' + p.humor_type + '</strong></div>';
      if (p.values_and_beliefs?.length) {
        html += '<div style="margin:8px 0"><div style="font-size:12px;
        color:var(--dim)">Core Values (' + p.values_and_beliefs.length + ')</div>';
        html += '<div class="voice-tags">';
        p.values_and_beliefs.slice(0,
        8).forEach(v => html += '<span class="voice-tag">' + esc(v.substring(0, 60)) + '</span>');
        if (p.values_and_beliefs.length > 8)
        html += '<span class="voice-tag">+' + (p.values_and_beliefs.length - 8) + ' more</span>';
        html += '</div></div>';
      }
      if (p.metaphor_families?.length) {
        html += '<div style="margin:8px 0"><div style="font-size:12px;
        color:var(--dim)">Metaphor Families</div>';
        html += '<div class="voice-tags">';
      ...
      if (p.taboos?.length) {
        html += '<div style="margin:8px 0"><div style="font-size:12px;
        color:var(--dim)">Taboos</div>';
        html += '<div class="voice-tags">';
        p.taboos.slice(0,
        6).forEach(t => html += '<span class="voice-tag" style="background:#2d0000;
        color:#fecaca">' + esc(t.substring(0, 50)) + '</span>');
        html += '</div></div>';
      }
      if (p.vocabulary_signature?.phrases?.length) {
        html += '<div style="margin:8px 0"><div style="font-size:12px;
        color:var(--dim)">Signature Phrases (' + p.vocabulary_signature.phrases.length + ')</div>';
        html += '<div class="voice-tags">';
        p.vocabulary_signature.phrases.slice(0,
        10).forEach(ph => html += '<span class="voice-tag" style="background:#1e1b4b;
        color:#c7d2fe">' + esc(ph) + '</span>');
        if (p.vocabulary_signature.phrases.length > 10)
        html += '<span class="voice-tag">+' + (p.vocabulary_signature.phrases.length - 10)
        + ' more</span>';
        html += '</div></div>';
      }
      html += '<div style="font-size:11px;color:var(--dim);
      margin-top:8px">Created: ' + new Date(p.created_at).toLocaleString() + '</div>';
      html += '</div>';
    }
    document.getElementById('voice-list').innerHTML = html;
  } catch (e) { document.getElementById('voice-list')
  .innerHTML = '<div class="empty">Error: ' + e.message + '</div>'; }
}
...
async function renderCreators() {
  const app = document.getElementById('app');
  app.innerHTML = '<div class="section"><h2>Creator Profiles</h2><div id="creators-list">
  <div class="empty">Loading...</div></div></div>';
  try {
    const creators = await api('/profiles/creators');
    if (!creators.length) { document.getElementById('creators-list')
    .innerHTML = '<div class="empty">No creators configured.</div>'; return; }
    let html = '<div class="grid">';
    for (const c of creators) {
      html += '<div class="card">';
      html += '<div style="display:flex;justify-content:space-between;
      align-items:center"><h3 style="text-transform:none">' + esc(c.creator_name) + '</h3>';
      html += '<span style="font-size:20px;font-weight:700;
      color:var(--accent)">' + ((c.allowed_influence_weight || 0.2) * 100).toFixed(0)
      + '%</span></div>';
      if (c.style_notes) html += '<p style="font-size:13px;color:var(--dim);
      margin-top:8px">' + esc(c.style_notes) + '</p>';
      ...
      if (c.voice_axes && Object.keys(c.voice_axes).length) {
        ...
        for (const [k, v] of Object.entries(c.voice_axes)) {
          ...
          background:var(--blue)"></div><span>' + v + '</span></div>';
        }
        html += '</div>';
      } else {
        html += '<div style="margin-top:12px;padding:12px;border:1px dashed #2a2d3a;
        border-radius:6px;text-align:center;color:#71717a;
        font-size:12px">No voice axes data yet.<br>
        <button class="btn btn-primary" style="margin-top:8px;padding:6px 14px;
        font-size:12px" onclick="analyzeVoice(\\'' + c.id + '\\')
        ">Analyze Voice from Posts</button></div>';
      }
      html += '</div>';
    }
    html += '</div>';
    document.getElementById('creators-list').innerHTML = html;
  } catch (e) { document.getElementById('creators-list')
  .innerHTML = '<div class="empty">Error: ' + e.message + '</div>'; }
}
...
async function renderAgents() {
  const app = document.getElementById('app');
  app.innerHTML = '<div class="section"><h2>Agent Model Configuration</h2><p style="color:var(--dim)
  ;font-size:13px;
  margin-bottom:16px">
  Change which LLM model each agent uses. Changes take effect immediately (no restart needed)
  .</p><div id="agents-list"><div class="empty">Loading...</div></div></div>';
  try {
    ...
    for (const a of agents) {
      ...
      justify-content:space-between;flex-wrap:wrap;gap:12px;padding:16px 20px">';
      ...
      for (const mid of MODEL_IDS) {
        ...
        html += '<button class="btn" style="' + (isActive ? 'background:' + mcolor + ';color:#000;
        font-weight:700' : 'background:var(--border);
        color:var(--dim)') + '" onclick="setAgentModel(\\'' + a.name + '\\',
        \\'' + mid + '\\')" title="' + MODEL_COSTS[mid] + '">' + mlabel + '</button>';
      }
      html += '</div>';
      html += '</div>';
    }
    html += '</div>';
    // Cost legend
    html += '<div class="card" style="margin-top:16px"><h3>Model Cost Reference</h3>
    <div style="display:flex;gap:24px;flex-wrap:wrap;margin-top:8px">';
    for (const mid of MODEL_IDS) {
      html += '<div style="font-size:13px"><span style="display:inline-block;width:10px;height:10px;
      border-radius:50%;background:' + MODEL_COLORS[mid] + ';
      margin-right:6px"></span><strong>' + MODEL_LABELS[mid] + '</strong>
      : ' + MODEL_COSTS[mid] + '</div>';
    }
    html += '</div></div>';
    document.getElementById('agents-list').innerHTML = html;
  } catch (e) { document.getElementById('agents-list')
  .innerHTML = '<div class="empty">Error: ' + e.message + '</div>'; }
}

async function setAgentModel(agent, model) {
  try {
    const r = await api('/admin/agents/' + agent + '/model', { method: 'PATCH',
...
async function renderCosts() {
  const app = document.getElementById('app');
  app.innerHTML = '<div class="section"><h2>Cost Dashboard</h2><div id="costs-content">
  <div class="empty">Loading cost data...</div></div></div>';
  try {
    ...
    html += '<div class="value" style="color:' + (daily.budget_pct_used > 80 ? 'var(--red)
    ' : daily.budget_pct_used > 50 ? 'var(--yellow)' : 'var(--green)')
    + '">$' + daily.total_cost_usd.toFixed(2) + '</div>';
    html += '<div class="sub">Budget: $' + daily.daily_budget_usd.toFixed(2)
    + ' (' + daily.budget_pct_used.toFixed(0) + '% used)</div>';
    html += '<div style="height:6px;background:var(--border);border-radius:3px;margin-top:8px;
    overflow:hidden"><div style="height:100%;
    background:' + (daily.budget_pct_used > 80 ? 'var(--red)
    ' : daily.budget_pct_used > 50 ? 'var(--yellow)' : 'var(--green)') + ';
    width:' + Math.min(daily.budget_pct_used, 100) + '%;border-radius:3px"></div></div>';
    html += '</div>';
    // This month
    html += '<div class="card"><h3>This Month</h3>';
    html += '<div class="value" style="color:' + (monthly.budget_pct_used > 80 ? 'var(--red)
    ' : monthly.budget_pct_used > 50 ? 'var(--yellow)' : 'var(--green)')
    + '">$' + monthly.total_cost_usd.toFixed(2) + '</div>';
    html += '<div class="sub">Budget: $' + monthly.monthly_budget_usd.toFixed(2)
    + ' (' + monthly.budget_pct_used.toFixed(0) + '% used)</div>';
    html += '<div style="height:6px;background:var(--border);border-radius:3px;margin-top:8px;
    overflow:hidden"><div style="height:100%;
    background:' + (monthly.budget_pct_used > 80 ? 'var(--red)
    ' : monthly.budget_pct_used > 50 ? 'var(--yellow)' : 'var(--green)') + ';
    width:' + Math.min(monthly.budget_pct_used, 100) + '%;border-radius:3px"></div></div>';
    html += '</div>';
    // Cost per post
    html += '<div class="card"><h3>Avg Cost/Post</h3>';
    html += '<div class="value">$' + perPost.avg_cost_per_run.toFixed(2) + '</div>';
    html += '<div class="sub">
    ' + perPost.total_runs + ' runs in ' + perPost.period_days + ' days</div>';
    if (perPost.total_runs > 0) html += '<div class="sub">Range: $' + perPost.min_cost.toFixed(2)
    + ' - $' + perPost.max_cost.toFixed(2) + '</div>';
    html += '</div>';
    // Total spent
    ...
    html += '<div class="card" style="margin-bottom:16px"><h3>
    Agent Cost Breakdown (' + byAgent.date + ')</h3>';
    if (byAgent.agents.length) {
      ...
      font-size:12px"><th style="text-align:left;
      padding:8px 12px">Agent</th><th style="text-align:right;
      padding:8px 12px">Cost</th><th style="text-align:right;
      padding:8px 12px">Input Tokens</th><th style="text-align:right;
      padding:8px 12px">Output Tokens</th><th style="text-align:right;
      padding:8px 12px">Calls</th></tr>';
      let totalCost = 0;
      for (const a of byAgent.agents) {
        totalCost += a.cost_usd;
        html += '<tr style="border-bottom:1px solid var(--border)">';
        html += '<td style="padding:8px 12px;
        font-weight:500">' + a.agent.replace(/_/g, ' ') + '</td>';
        html += '<td style="text-align:right;padding:8px 12px;color:var(--accent2);
        font-weight:600">$' + a.cost_usd.toFixed(4) + '</td>';
        html += '<td style="text-align:right;padding:8px 12px;
        color:var(--dim)">' + (a.input_tokens || 0).toLocaleString() + '</td>';
        html += '<td style="text-align:right;padding:8px 12px;
        color:var(--dim)">' + (a.output_tokens || 0).toLocaleString() + '</td>';
        html += '<td style="text-align:right;padding:8px 12px">' + a.call_count + '</td>';
        html += '</tr>';
      }
      html += '<tr style="font-weight:700"><td style="padding:8px 12px">Total</td>
      <td style="text-align:right;padding:8px 12px;
      color:var(--green)">$' + totalCost.toFixed(4) + '</td><td colspan="3"></td></tr>';
      html += '</table>';
    } else {
      html += '<div class="empty" style="padding:16px">
      No cost data for today. Run a pipeline to see costs.</div>';
    }
    html += '</div>';
    // Model distribution
    html += '<div class="card"><h3>Model Distribution (Last ' + modelDist.period_days + ' Days)
    </h3>';
    if (modelDist.models.length) {
      ...
      for (const m of modelDist.models) {
        ...
        html += '<div style="flex:1;min-width:200px;background:#111318;
        border:1px solid var(--border);border-radius:8px;padding:16px">';
        html += '<div style="display:flex;align-items:center;gap:8px;
        margin-bottom:8px"><span style="width:12px;height:12px;border-radius:50%;
        background:' + color + ';
        display:inline-block"></span><span style="font-weight:600">' + label + '</span></div>';
        html += '<div style="font-size:22px;font-weight:700;
        color:' + color + '">$' + m.cost_usd.toFixed(2) + '</div>';
        html += '<div style="font-size:12px;color:var(--dim);
        margin-top:4px">' + pct.toFixed(0) + '% of total - ' + m.call_count + ' calls</div>';
        html += '<div style="font-size:12px;
        color:var(--dim)">' + (m.input_tokens || 0).toLocaleString()
        + ' in / ' + (m.output_tokens || 0).toLocaleString() + ' out</div>';
        html += '</div>';
      }
      html += '</div>';
    } else {
    ...
    try {
      ...
      if (runs.length) {
        ...
        html += '<tr style="border-bottom:1px solid var(--border);color:var(--dim);
        font-size:12px"><th style="text-align:left;
        padding:8px 12px">Run ID</th><th style="text-align:left;
        padding:8px 12px">Workflow</th><th style="text-align:left;
        padding:8px 12px">Status</th><th style="text-align:left;
        padding:8px 12px">Day</th><th style="text-align:left;
        padding:8px 12px">Started</th><th style="text-align:left;padding:8px 12px">Error</th></tr>';
        for (const r of runs) {
          const stColor = r.status === 'completed' ? 'var(--green)
          ' : r.status === 'failed' ? 'var(--red)' : 'var(--blue)';
          html += '<tr style="border-bottom:1px solid var(--border)">';
          html += '<td style="padding:8px 12px;font-family:monospace;
          font-size:12px">' + r.run_id.substring(0, 8) + '</td>';
          html += '<td style="padding:8px 12px">' + r.workflow + '</td>';
          html += '<td style="padding:8px 12px;color:' + stColor + ';
          font-weight:600">' + r.status + '</td>';
          html += '<td style="padding:8px 12px">
          ' + (r.day_of_week != null ? DAY_NAMES[r.day_of_week] || r.day_of_week : '-') + '</td>';
          html += '<td style="padding:8px 12px;
          color:var(--dim)">' + (r.started_at ? new Date(r.started_at).toLocaleString() : '-')
          + '</td>';
          html += '<td style="padding:8px 12px;color:var(--red);font-size:12px;max-width:200px;
          overflow:hidden;text-overflow:ellipsis;
          white-space:nowrap" title="' + esc(r.error_message || '')
          + '">' + esc((r.error_message || '').substring(0, 80)) + '</td>';
          html += '</tr>';
        }
        html += '</table></div>';
      }
    ...
    document.getElementById('costs-content')
    .innerHTML = '<div class="empty">Error loading costs: ' + e.message + '</div>';
  }
}
...
function showFeedbackForm(packageId, btn) {
  ...
  let html = '<div id="feedback-form-' + packageId + '" style="background:#111318;
  border:1px solid var(--accent);border-radius:8px;padding:16px;margin-top:12px">';
  html += '<div style="font-size:14px;font-weight:600;color:var(--accent2);
  margin-bottom:12px">Package Feedback</div>';
  html += '<div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:12px">';
  for (const tag of FEEDBACK_TAGS) {
    const isPositive = tag.startsWith('great_') || tag.startsWith('strong_')
    || tag.startsWith('perfect_') || tag.startsWith('good_');
    ...
    background:var(--card)">';
    html += '<input type="checkbox" class="fb-tag" value="' + tag + '" style="accent-color:' +
    (isPositive ? 'var(--green)' : 'var(--yellow)') + '"> ';
    html += '<span style="color:' + (isPositive ? 'var(--green)' : 'var(--text)')
    + '">' + tag.replace(/_/g, ' ') + '</span>';
    html += '</label>';
  }
  html += '</div>';
  html += '<textarea id="fb-notes-' + packageId + '" placeholder="Additional notes (optional)
  ..." style="width:100%;height:60px;background:var(--card);border:1px solid var(--border);
  color:var(--text);border-radius:6px;padding:8px;font-size:13px;resize:vertical"></textarea>';
  html += '<div style="display:flex;gap:8px;margin-top:8px">';
  html += '<select id="fb-action-' + packageId + '" style="padding:6px 10px">
  <option value="approved">Approved</option><option value="revised">Revised</option>
  <option value="rejected">Rejected</option></select>';
  html += '<button class="btn btn-primary" onclick="submitFeedback(\\'' + packageId + '\\')
  ">Submit Feedback</button>';
  html += '<button class="btn btn-dim" onclick="document.getElementById(\\'feedback-form-' +
  packageId + '\\').remove()">Cancel</button>';
  html += '</div></div>';
...
function showRevisedCopyForm(packageId, btn) {
  ...
  let html = '<div id="revised-form-' + packageId + '" style="background:#111318;
  border:1px solid var(--accent);border-radius:8px;padding:16px;margin-top:12px">';
  html += '<div style="font-size:14px;font-weight:600;color:var(--accent2);
  margin-bottom:4px">Submit Revised Copy</div>';
  html += '<div style="font-size:12px;color:var(--dim);
  margin-bottom:12px">
  Paste what you actually published. The system learns from your edits to match your voice.</div>';
  // Facebook
  html += '<div style="margin-bottom:12px">';
  html += '<div style="display:flex;justify-content:space-between;align-items:center;
  margin-bottom:4px"><label style="font-size:13px;font-weight:600;
  color:var(--green)
  ">Facebook Post</label><span id="rev-fb-wc-' + packageId + '" style="font-size:11px;
  color:var(--dim)">' + fbWords + ' words</span></div>';
  html += '<textarea id="rev-fb-' + packageId + '" style="width:100%;min-height:200px;
  background:var(--card);border:1px solid var(--border);color:var(--text);border-radius:6px;
  padding:8px;font-size:13px;resize:vertical;
  line-height:1.5" oninput="updateWordCount(this,
  \\'rev-fb-wc-' + packageId + '\\')">' + esc(fbText) + '</textarea>';
  html += '</div>';
  // LinkedIn
  html += '<div style="margin-bottom:12px">';
  html += '<div style="display:flex;justify-content:space-between;align-items:center;
  margin-bottom:4px"><label style="font-size:13px;font-weight:600;
  color:var(--blue,
  #3b82f6)">LinkedIn Post</label><span id="rev-li-wc-' + packageId + '" style="font-size:11px;
  color:var(--dim)">' + liWords + ' words</span></div>';
  html += '<textarea id="rev-li-' + packageId + '" style="width:100%;min-height:200px;
  background:var(--card);border:1px solid var(--border);color:var(--text);border-radius:6px;
  padding:8px;font-size:13px;resize:vertical;
  line-height:1.5" oninput="updateWordCount(this,
  \\'rev-li-wc-' + packageId + '\\')">' + esc(liText) + '</textarea>';
  html += '</div>';
  // Diff preview area
  html += '<div id="rev-diff-' + packageId + '" style="display:none;margin-bottom:12px"></div>';
  // Buttons
  html += '<div style="display:flex;gap:8px">';
  html += '<button class="btn btn-primary" onclick="submitRevisedCopy(\\'' + packageId + '\\')
  ">Submit Revised Copy</button>';
  html += '<button class="btn btn-dim" onclick="previewDiff(\\'' + packageId + '\\')
  ">Preview Changes</button>';
  html += '<button class="btn btn-dim" onclick="document.getElementById(\\'revised-form-' +
  packageId + '\\').remove()">Cancel</button>';
  html += '</div></div>';
...
function previewDiff(packageId) {
  ...
  let html = '';
  if (origFb !== newFb) {
    html += '<div style="margin-bottom:8px"><strong style="color:var(--green);
    font-size:12px">Facebook changes:</strong></div>';
    html += '<div style="font-size:12px;background:#0d1117;padding:10px;border-radius:6px;
    border:1px solid var(--border);margin-bottom:12px">' + wordDiff(origFb, newFb) + '</div>';
  }
  if (origLi !== newLi) {
    html += '<div style="margin-bottom:8px"><strong style="color:#3b82f6;
    font-size:12px">LinkedIn changes:</strong></div>';
    html += '<div style="font-size:12px;background:#0d1117;padding:10px;border-radius:6px;
    border:1px solid var(--border);margin-bottom:12px">' + wordDiff(origLi, newLi) + '</div>';
  }
  if (!html) html = '<div style="font-size:12px;color:var(--dim);
  padding:8px">No changes detected.</div>';
...
function wordDiff(oldText, newText) {
  ...
  if (m > 500 || n > 500) {
    ...
    text-decoration:line-through">' + esc(oldText.slice(0,
    200)) + '...</span> <span style="color:var(--green)">' + esc(newText.slice(0,
    200)) + '...</span>';
  }
  const dp = Array.from({length: m + 1}, () => new Array(n + 1).fill(0));
  for (let i = 1; i <= m; i++) for (let j = 1; j <= n; j++) {
    dp[i][j] = oldWords[i-1] === newWords[j-1] ? dp[i-1][j-1] + 1 : Math.max(dp[i-1][j],
    dp[i][j-1]);
  }
  const result = [];
  let i = m, j = n;
  while (i > 0 || j > 0) {
    if (i > 0 && j > 0 && oldWords[i-1] === newWords[j-1]) {
      ...
      result.unshift('<span style="color:var(--green);
      background:#05200e">' + esc(newWords[j-1]) + '</span>');
      j--;
    } else {
      result.unshift('<span style="color:var(--red);text-decoration:line-through;
      background:#200505">' + esc(oldWords[i-1]) + '</span>');
      i--;
    }
  }
  return result.join('');
...
async function submitRevisedCopy(packageId) {
  ...
  try {
    ...
    submitBtn.style.background = 'var(--green)'; submitBtn.style.color = '#000'; }
...
async function renderAnalytics() {
  const app = document.getElementById('app');
  app.innerHTML = '<div class="section"><h2>Analytics & Performance</h2><div id="analytics-content">
  <div class="empty">Loading analytics...</div></div></div>';
  try {
    ...
    if (failedRuns) html += '<div class="sub" style="color:var(--red)
    ">' + failedRuns + ' failed</div>';
    html += '</div>';
    html += '<div class="card"><h3>Feedback Submitted</h3><div class="value">' + fbCount + '</div>';
    html += '<div class="sub">Operator reviews</div></div>';
    html += '<div class="card"><h3>Learning Events</h3><div class="value">
    ' + learning.length + '</div>';
    ...
    if (pkgs.length) {
      ...
      html += '<tr style="border-bottom:1px solid var(--border);color:var(--dim);
      font-size:12px"><th style="text-align:left;
      padding:8px 12px">Date</th><th style="text-align:left;
      padding:8px 12px">Status</th><th style="text-align:left;
      padding:8px 12px">CTA</th><th style="text-align:left;
      padding:8px 12px">QA Score</th><th style="text-align:left;
      padding:8px 12px">Feedback</th></tr>';
      for (const p of pkgs.slice(0, 15)) {
        const stColor = p.approval_status === 'approved' ? 'var(--green)
        ' : p.approval_status === 'rejected' ? 'var(--red)' : 'var(--yellow)';
        ...
        html += '<tr style="border-bottom:1px solid var(--border)">';
        html += '<td style="padding:8px 12px">' + new Date(p.created_at).toLocaleDateString()
        + '</td>';
        html += '<td style="padding:8px 12px;color:' + stColor + ';
        font-weight:600">' + p.approval_status + '</td>';
        html += '<td style="padding:8px 12px">' + esc(p.cta_keyword || '-') + '</td>';
        html += '<td style="padding:8px 12px;font-weight:600">' + qaVal + '</td>';
        html += '<td style="padding:8px 12px">' + (fb ? '<span style="color:var(--green)
        ">Yes</span>' : '<span style="color:var(--dim)">-</span>') + '</td>';
        html += '</tr>';
      }
      html += '</table>';
    } else {
    ...
    if (feedback.length) {
      ...
      if (Object.keys(tagCounts).length) {
        ...
        for (const [tag, count] of sorted) {
          const isPositive = tag.startsWith('great_') || tag.startsWith('strong_')
          || tag.startsWith('perfect_') || tag.startsWith('good_');
          html += '<div style="padding:6px 12px;border-radius:6px;font-size:13px;
          background:' + (isPositive ? '#052e16' : '#2d2000') + ';
          color:' + (isPositive ? 'var(--green)' : 'var(--yellow)') + '">';
    ...
    ;
    html += '<div class="card" style="margin-bottom:16px"><h3 style="color:var(--accent2)
    ">Voice Learning</h3>';
    html += '<div style="margin-top:12px">';
    if (revisedFeedback.length) {
      html += '<div style="display:flex;gap:16px;margin-bottom:12px">';
      html += '<div style="padding:12px 20px;background:#0d1117;border-radius:8px;
      border:1px solid var(--border);text-align:center">';
      html += '<div style="font-size:24px;font-weight:700;
      color:var(--accent2)">' + revisedFeedback.length + '</div>';
      ...
      html += '</div>';
      html += '<div style="font-size:12px;color:var(--dim);padding:8px;background:#0d1117;
      border-radius:6px">
      The Learning Loop agent analyzes your edits to identify voice patterns and adjust future
      content generation. More revisions = smarter content.</div>';
    } else {
      html += '<div class="empty" style="padding:16px">
      No revised copies submitted yet. Use the "Edit & Submit Copy" button on packages to teach the
      system your voice.</div>';
    }
    html += '</div></div>';
    ...
    if (pkgsWithDm.length) {
      ...
      html += '<tr style="border-bottom:1px solid var(--border);color:var(--dim);
      font-size:12px"><th style="text-align:left;
      padding:8px 12px">Package Date</th><th style="text-align:left;
      padding:8px 12px">CTA Keyword</th><th style="text-align:left;
      padding:8px 12px">Trigger</th><th style="text-align:left;
      padding:8px 12px">Has DM Flow</th></tr>';
      for (const p of pkgsWithDm.slice(0, 10)) {
        const dm = p.dm_flow || {};
        html += '<tr style="border-bottom:1px solid var(--border)">';
        html += '<td style="padding:8px 12px">' + new Date(p.created_at).toLocaleDateString()
        + '</td>';
        html += '<td style="padding:8px 12px;font-weight:600;
        color:var(--accent2)">' + esc(p.cta_keyword || '-') + '</td>';
        html += '<td style="padding:8px 12px">' + esc(dm.trigger || '-') + '</td>';
        html += '<td style="padding:8px 12px;
        color:var(--green)">Yes - ' + Object.keys(dm).length + ' steps</td>';
        html += '</tr>';
      }
      html += '</table>';
    } else {
      html += '<div class="empty" style="padding:16px">
      No packages with DM flows yet. DM flows are generated by the CTA Agent during pipeline
      runs.</div>';
    }
    html += '</div></div>';
    ...
    .innerHTML = '<div class="empty">Error loading analytics: ' + e.message + '</div>';
  }
}

// Router
function render() {
  const map = { week: renderWeek, generate: renderGenerate, packages: renderPackages,
  corpus: renderCorpus, voice: renderVoice, creators: renderCreators, agents: renderAgents,
  costs: renderCosts, analytics: renderAnalytics };
