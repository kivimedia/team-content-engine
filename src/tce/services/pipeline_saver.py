...
class PipelineResultSaver:
    ...
    async def save_post_examples(
        ...
        creator_map = context.get("creator_map", {})
        created_ids: list[uuid.UUID] = []

        for ex in examples:
            creator_name = ex.get("creator_name", "unknown")
            creator_id = creator_map.get(creator_name)
            if not creator_id and document_id:
                # Fall back to document_id context if no creator mapping
                creator_id = context.get("default_creator_id")
            if not creator_id:
                logger.warning(
                    "saver.skip_example",
                    reason="no creator_id",
