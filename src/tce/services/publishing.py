...
class FacebookAdapter(PublishingAdapter):
    """Facebook publishing adapter - gated by feature flag."""
    ...
    async def publish(self, package: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            async with async_session() as db:
                ...
                enabled = await flags.is_enabled("publish_facebook")
        except Exception:
            enabled = False

        if not enabled:
            logger.info("facebook.disabled_by_flag")
            return {
                "adapter": "facebook",
                "status": "disabled",
        ...
        # TODO: Implement Facebook Graph API publish when ready
        return {
            "adapter": "facebook",
...
class LinkedInAdapter(PublishingAdapter):
    """LinkedIn publishing adapter - gated by feature flag."""
    ...
    async def publish(self, package: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            async with async_session() as db:
                flags = FeatureFlagService(db)
                enabled = await flags.is_enabled("publish_linkedin")
        except Exception:
            enabled = False

        if not enabled:
            logger.info("linkedin.disabled_by_flag")
            return {
                "adapter": "linkedin",
                "status": "disabled",
        ...
        # TODO: Implement LinkedIn API publish when ready
        return {
            "adapter": "linkedin",
            "status": "not_implemented",
            "message": (
